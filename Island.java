...
public class Island {
    ...
    private void populateIsland() {
        ...
        // Добавляем животных случайным образом
    ...
    public void startSimulation() {
        ...
        // Задача для роста растений (каждые 3 такта)
        scheduler.scheduleAtFixedRate(this::growPlantsTask, 0, 3, TimeUnit.SECONDS);

        // Задача для жизненного цикла животных (каждый такт)
    ...
    private void processCell(int x, int y) {
        ...
        // Перемешиваем для случайного порядка обработки
        ...
        for (Animal animal : animals) {
            ...
            if (animal instanceof Predator) {
                // Хищник пытается съесть других животных
                ...
                // Травоядное ест растения
                if (cell.getPlantsCount() > 0) {
                    animal.eat(new Plant(), cell);
                }
                // Некоторые травоядные едят других животных (утка, кабан)
                for (Animal prey : animals) {
                    if (prey != animal && prey.isAlive() && animal.eat(prey, cell)) {
                        break;
                    }
